/* cgi-lib.c - C routines that make writing CGI scripts in C a breeze
   ...
   $Id: cgi-lib.c,v 1.1.1.1 2004/12/19 07:33:06 Nik Exp $
  ...
  04-July-2000
  Modified by Nikolai Golovchenko <golovchenko@mail.ru>
...
char *cgi_val(llist l, char *name)
{
  ...
  while ( (window != 0) && (!FOUND) )
    if (!strcasecmp(window->entry.name,name)) //case-insensitive
      FOUND = 1;
    else
      window = window->next;
  if (FOUND)
